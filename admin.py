...
def billing_page():
    ...
    with st.expander("Add Products", expanded=True):
        ...
        selected_category = st.selectbox("Category", ["Select"] + categories, key="category")
        
        if selected_category != "Select":
            subcategories = get_subcategories(selected_category)
            selected_subcat = st.selectbox("Subcategory", ["Select"] + subcategories, key="subcategory")
            
            if selected_subcat != "Select":
                products = get_products(selected_category, selected_subcat)
                selected_product = st.selectbox("Product", ["Select"] + products, key="product")
